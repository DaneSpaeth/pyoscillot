...
def _create_and_save_arrays_pulsation():
    ...
    for p in phases:
        star = ThreeDimStar()
        projector = TwoDimProjector(star, inclination=60, line_of_sight=True)
        star.add_pulsation(l=2, m=2, t=p * P)
        np.save(f"arrays/pulsation_all/{round(p,3)}",
                -1 * projector.pulsation())


def read_in_saved_arrays_pulsation():
    path = Path("arrays/pulsation_all")
...
def animate_pulse():
    ...
    # Read in data
    sim_star = "pulsation_inclination60"
    ...
    # Create the plot at steady state


def init_plots(ax, index, time, rv, rve, crx, crxe, dlw=None, dlwe=None):
    """ Init plots."""
...
def update_plots(ax, index, time, rv, rve, crx, crxe, dlw=None, dlwe=None):
    """ Init plots."""
...
def animate_spot():
    ...
    # Initialize the plot part
    ax = update_plots(ax, index + 1, time, rv, rve, crx, crxe)
