...
if __name__ == "__main__":
    ...
    markersize = 1
    ax.plot(wave, ref_spectra[4500][1.0], marker="^", markersize=markersize, label="$T_\mathrm{eff}=4500\,\mathrm{K}$ (PHOENIX)")
    ax.plot(wave, ref_spectra[4600][1.0], alpha=0.7, marker="v", markersize=markersize, label="$T_\mathrm{eff}=4600\,\mathrm{K}$ (PHOENIX)")
    ax.plot(wave, spec_interpol, alpha=0.7, marker="o", markersize=markersize, label="$T_\mathrm{eff}=4550\,\mathrm{K}$ (interpolated)")
    ...
    plt.savefig(out_root / "temp_interpolation.pdf", dpi=300)
